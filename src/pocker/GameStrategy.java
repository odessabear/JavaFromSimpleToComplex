...
public class GameStrategy implements ComparisonStrategy{
    ...
    public int compare(Hand thisHand, Hand thatHand) {
        if (thisHand.calcHandValue() == thatHand.calcHandValue()){
            ...
            throw new IllegalArgumentException("This comparison approach should be used only for the hands with even value");
        }
    }
    ...
    private int compareHands(Hand thisHand, Hand thatHand){
        ...
        thisHandSet.addAll(thisHand.getCardSet().stream().collect(groupingBy(Card::getRank)).entrySet());

        SortedSet<Map.Entry<Rank, List<Card>>> thatHandSet = new TreeSet<>(new HandComparator());
        thatHandSet.addAll(thatHand.getCardSet().stream().collect(groupingBy(Card::getRank)).entrySet());
